...
\umlusecase[x=0,y=1]{case2}
\umlactor{User}
\umlactor{Company Administrator}
\umlactor{Customer Service}
\umlactor{Package Maintainer}
\umlactor{Security Engineer}
\end{tikzpicture}

\subsubsection{Fedora}
\begin{tikzpicture}
\umlactor{User}
\umlusecase{Report a problem}
\umlactor{Package Maintainer}
\umlactor{Upstream Developer}
\umlactor{Security Engineer}
\end{tikzpicture}
...
% Above report left
\umlclass[x=-6,y=3]{report os release}{
report id : FOREIGN KEY \\
operating system release id : FOREIGN KEY \\
count : INT
}{}

\umlclass[x=-5,y=6]{report selinux state}{
report id : FOREIGN KEY \\
selinux state : ENUM \\
count : INT
}{}

\umlclass[x=-5,y=9]{report selinux context}{
report id : FOREIGN KEY \\
context : STRING NOT NULL \\
count : INT
}{}

\umlclass[x=-5,y=12]{report duplicate hash}{
report id : FOREIGN KEY \\
hash : STRING \\
type : ENUM (NAMES, HASHES)
}{}

\umluniaggreg[geometry=|-,pos1=0.2,mult1=1,mult2=+,pos2=1.9]{report}{report os release}
\umluniaggreg[geometry=|-,                 mult2=+,pos2=1.9]{report}{report selinux state}
\umluniaggreg[geometry=|-,                 mult2=+,pos2=1.9]{report}{report selinux context}
\umluniaggreg[geometry=|-,                 mult2=+,pos2=1.9]{report}{report duplicate hash}

% Above report right
\umlclass[x=4,y=3]{report binary}{
id : PRIMARY KEY \\
path : STRING \\
count : INT
}{}
...
\umluniaggreg[geometry=|-,mult2=+,pos2=1.9]{report}{report binary}
\umluniaggreg[geometry=|-,mult2=+,pos2=1.9]{report}{report proc status}

% Below report left
\umlclass[x=-5,y=-4]{report package}{
report id : FOREIGN KEY \\
package id : FOREIGN KEY \\
updated from package id : FOREIGN KEY \\
count : INT
}{}

\umlclass[x=-5,y=-7]{report related package}{
report id : FOREIGN KEY \\
package id : FOREIGN KEY \\
count : INT
}{}
...
\texttt{crashtime} & Unsigned integer. & yes & POSIX time: number of seconds elapsed since midnight UTC of January 1, 1970. \\
...
\texttt{package} & Dictionary; see the corresponding table below. & yes & \\
\texttt{related\_packages} & List of dictionaries; see the \texttt{package} description below. & yes & \\
\texttt{os} & & yes & \\
\texttt{architecture} & \texttt{x86\_64} or \texttt{i386} & yes & \\
\texttt{reporter} & Dictionary; see the corresponding table below. & yes & Program that created the report. \\
...
\texttt{selinux} & Dictionary; see the corresponding table below. & no & SELinux presence and state. \\
...
\hline
Name & Format & Mandatory & Notes \\ \hline
\texttt{state} & \texttt{enforcing}, \texttt{permissive}, or \texttt{disabled} & yes & \\
\texttt{process} & ASCII string, max. 128 characters & yes, if the \texttt{state} is either \texttt{enforcing} or \texttt{permissive} & \texttt{ps -e --context} \\
